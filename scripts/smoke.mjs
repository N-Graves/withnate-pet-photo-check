...
const JS = new URL("../dist/pet-photo-check.js", import.meta.url);
const CSS = new URL("../dist/pet-photo-check.css", import.meta.url);
...
const foreign = [...new Set(selectors.map((s) => s.trim()))].filter((s) => !s.startsWith(".ppc"));
check(foreign.length === 0, "defines only .ppc- classes", foreign.join(" "));
